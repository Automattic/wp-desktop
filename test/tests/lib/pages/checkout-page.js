...
class CheckoutPage extends AsyncBaseContainer {
	...
	async isShoppingCartPresent() {
		await driverHelper.waitTillPresentAndDisplayed( this.driver, By.css( '.payment-box__content' ) );
	...
	async emptyShoppingCart() {
		...
		await driverHelper.isEventuallyPresentAndDisplayed( this.driver, By.css( '#plans' ) );
	}
}
